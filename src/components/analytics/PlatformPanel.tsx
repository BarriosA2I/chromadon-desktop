...
export function PlatformPanel() {
  ...
  const data = analyticsData.platforms[selectedPlatform]
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-1.5 flex-wrap">
        {PLATFORMS.map(p => {
          const hasData = !!analyticsData.platforms[p]
          ...
          return (
            ...
            >
              ...
              {p}
              {!hasData && <span className="w-1 h-1 rounded-full bg-gray-600 inline-block" />}
            </button>
          )
        })}
      </div>
      ...
      {!data ? (
        <div className="flex flex-col items-center justify-center h-48 text-gray-500">
          <div className="w-12 h-12 rounded-xl border border-white/[0.08] bg-white/[0.03] flex items-center justify-center mb-3">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={PLATFORM_COLORS[selectedPlatform] || '#666'} strokeWidth="1.5" opacity="0.5">
              <path d={PLATFORM_ICONS[selectedPlatform] || ''} />
            </svg>
          </div>
          <div className="text-sm font-medium text-gray-400">No data for {selectedPlatform}</div>
          <div className="text-[11px] text-gray-600 mt-1 font-mono">Run data collection for this platform</div>
        </div>
      ) : (
        <>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            <MetricCard label="Followers" value={data.followers} color={PLATFORM_COLORS[selectedPlatform]} index={0} />
            <MetricCard label="Following" value={data.following} index={1} />
            <MetricCard label="Growth Rate" value={data.growthRate} format="percent" color="#10B981" index={2} />
            <MetricCard label="Posts" value={data.posts.length} color="#D4AF37" index={3} />
          </div>
          ...
          {data.topPosts.length > 0 && (
            ...
            >
              ...
              <div className="flex items-center gap-2 mb-3">
                ...
                <h3 className="text-[11px] font-bold uppercase tracking-widest text-gray-400">Top Posts</h3>
              </div>
              <div className="space-y-2">
                {data.topPosts.slice(0, 5).map((post, i) => (
                  <div key={i} className="flex items-start justify-between text-xs border-b border-white/[0.04] pb-2">
                    <div className="flex items-start gap-2 flex-1 mr-3">
                      <span className="text-gray-600 font-mono text-[10px] mt-0.5">{String(i + 1).padStart(2, '0')}</span>
                      <p className="text-gray-300 line-clamp-1">{post.content}</p>
                    </div>
                    <div className="flex gap-3 text-gray-500 whitespace-nowrap font-mono text-[11px]">
                      ...
                      <span className="text-[#00CED1]/60">{formatPercent(post.metrics?.engagement_rate || 0)}</span>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {data.audienceHistory.length > 1 && (
            ...
            >
              ...
              <div className="flex items-center gap-2 mb-3">
                <div className="w-1 h-3 rounded-full" style={{ backgroundColor: PLATFORM_COLORS[selectedPlatform] }} />
                <h3 className="text-[11px] font-bold uppercase tracking-widest text-gray-400">Audience History</h3>
              </div>
              <TrendChart
                data={data.audienceHistory.map(h => ({
                  date: h.collected_at,
                  followers: h.followers,
                }))}
                lines={[{ dataKey: 'followers', name: 'Followers', color: PLATFORM_COLORS[selectedPlatform] }]}
                height={180}
              />
            </motion.div>
          )}
